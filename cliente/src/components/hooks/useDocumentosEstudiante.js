...
const useDocumentosEstudiante = (tipoProceso, procesoIdProp) => {
  ...
  const fetchPlantillas = async () => {
    ...
    setError(null);
    try {
      const response = await api.get("/documentosAdmin"); // ✅ Cambiado
  ...
  const fetchDocumentos = async () => {
    ...
    setError(null);
    try {
      const { data } = await api.get("/documentos", { // ✅ Cambiado
  ...
  const uploadDocumento = async (idTipoDoc, file) => {
    ...
    // ✅ ELIMINADO: formData.append("id_usuario", user.id);
    // ✅ ELIMINADO: formData.append("Estatus", "Pendiente");
    formData.append("Comentarios", "");
    formData.append("id_proceso", procesoId);

    try {
      await api.post("/documentos/upload", formData, { // ✅ Cambiado
  ...
  const deleteDocumento = async (idDocumento) => {
    ...
    setSuccess(null);
    try {
      await api.delete(`/documentos/${idDocumento}`); // ✅ Cambiado
