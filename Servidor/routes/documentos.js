 ...
 * - Admin: acceso total a todos los documentos
...
const verificarPermisoDocumento = async (req, res, next) => {
  try {
    ...
    // ❌ Si NO es admin, verificar que el documento le pertenece
    ...
    // Verificar que el usuario es dueño del documento o del proceso
    if (documento[0].estudiante_user_id !== userId) {
      return res.status(403).json({ error: "No tienes permiso para acceder a este documento" });
    }

    next();
  } catch (error) {
    console.error('Error al verificar permisos de documento:', error);
    res.status(500).json({ error: "Error al verificar permisos" });
  }
};

/**
 * Verificar que solo admin puede aprobar/rechazar/revertir documentos
 */
...
// ============================================================================
// RUTAS DE CATÁLOGOS (solo requieren autenticación válida)
// ============================================================================
...
router.get("/", authenticateToken, async (req, res) => {
  try {
    ...
    // ✅ Si ES admin, puede ver TODOS los documentos (no agregamos condición)
...
router.post("/upload", authenticateToken, upload.single("archivo"), async (req, res) => {
  try {
    const { IdTipoDoc, Comentarios = "", Estatus = "Pendiente", id_proceso } = req.body;
    const file = req.file;
    ...
    // ✅ Verificar permisos: Admin puede subir a cualquier proceso, estudiante solo al suyo
    if (userRole !== 'admin') {
    ...
    // Verificar si ya existe un documento del mismo tipo en el proceso
...
// ============================================================================
// DESCARGAR DOCUMENTO
// Admin: puede descargar todos
// Estudiante: solo puede descargar sus propios documentos
// ============================================================================
...
router.put("/approve/:id_Documento", authenticateToken, soloAdmin, async (req, res) => {
  try {
    const { id_Documento } = req.params;
...
router.put("/reject/:id_Documento", authenticateToken, soloAdmin, async (req, res) => {
  try {
    const { id_Documento } = req.params;
...
router.put("/revert/:id_Documento", authenticateToken, soloAdmin, async (req, res) => {
  try {
    const { id_Documento } = req.params;
...
// Estudiante: solo puede eliminar sus propios documentos
// ============================================================================
