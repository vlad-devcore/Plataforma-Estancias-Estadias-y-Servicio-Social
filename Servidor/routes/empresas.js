...
const postEmpresa = async (req, res) => {
    const { empresa_rfc, empresa_nombre, empresa_direccion, empresa_email, empresa_telefono, empresa_tamano, empresa_sociedad, empresa_pagina_web } = req.body;
    ...
    if (!tamanosPermitidos.includes(empresa_tamano)) {
        return res.status(400).json({ error: "Valor no válido para empresa_tamano. Los valores permitidos son: 'Grande', 'Mediana', 'Pequeña'" });
    }
    ...
    if (!sociedadesPermitidas.includes(empresa_sociedad)) {
        return res.status(400).json({ error: "Valor no válido para empresa_sociedad. Los valores permitidos son: 'Privada', 'Publica'" });
    }
    ...
    try {
        ...
        if (existingEmpresa.length > 0) {
            return res.status(400).json({ error: "El RFC de la empresa ya está registrado" });
        }
        ...
        res.status(201).json({ message: "Empresa añadida correctamente", id_empresa: results.insertId });
...
const updateEmpresa = async (req, res) => {
    ...
    try {
        ...
        if (existingEmpresa.length > 0) {
            return res.status(400).json({ error: "El RFC de la empresa ya está registrado en otra empresa" });
        }
        ...
        const [results] = await connection.query(
            ...
            [empresa_rfc, empresa_nombre, empresa_direccion, empresa_email, empresa_telefono, empresa_tamano, empresa_sociedad, empresa_pagina_web, id_empresa]
        );
